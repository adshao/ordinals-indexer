...
// The InscriptionFunc type is an adapter to allow the use of ordinary
// function as Inscription mutator.
type InscriptionFunc func(context.Context, *ent.InscriptionMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f InscriptionFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.InscriptionMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.InscriptionMutation", m)
}
