...
func (tq *TokenQuery) QueryCollection() *CollectionQuery {
	query := (&CollectionClient{config: tq.config}).Query()
...
func (tq *TokenQuery) Clone() *TokenQuery {
	...
	return &TokenQuery{
		...
		withCollection: tq.withCollection.Clone(),
...
func (tq *TokenQuery) WithCollection(opts ...func(*CollectionQuery)) *TokenQuery {
	query := (&CollectionClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withCollection = query
	return tq
}
...
func (tq *TokenQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Token, error) {
	var (
		...
		_spec       = tq.querySpec()
		loadedTypes = [1]bool{
			tq.withCollection != nil,
		}
	)
	if tq.withCollection != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := tq.withCollection; query != nil {
		if err := tq.loadCollection(ctx, query, nodes, nil,
			func(n *Token, e *Collection) { n.Edges.Collection = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TokenQuery) loadCollection(ctx context.Context, query *CollectionQuery, nodes []*Token, init func(*Token), assign func(*Token, *Collection)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Token)
	for i := range nodes {
		if nodes[i].collection_tokens == nil {
			continue
		}
		fk := *nodes[i].collection_tokens
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "collection_tokens" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
